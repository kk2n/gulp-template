...
gulp.task('?', function () {
    ...
    console.log('gulp concat-css                上一步编译后，合并、压缩css');
    console.log('gulp rev-css                   修改html中css的MD5参数');
    console.log('gulp watch-css                 监控css,有变化[concat-css],[rev-css]');
    console.log('gulp watch-html                监控HTML文件，只要有变化，就copy');
    console.log('gulp watch                     文件监控=等于sass,watch-sass,watch-css\n' +
        '                               watch-css又等于clean-css,concat-css，rev-css');
    console.log('+-------------------------------------------------------------------+');
    console.log('+                                                                   +');
    console.log('+                                                                   +');
...
gulp.task('concat-js', function () {
    return gulp.src(JsPlugins.InFile) // 匹配文件
        ...
        .pipe(gulp.dest(Url.App.Js));  // 输出到'build/js/'下
});
//合并js，并进行压缩
gulp.task('min-js', function () {
    return gulp.src(Url.App.Js + '/' + JsPlugins.OutFile.Concat) // 匹配文件//
        .pipe(plugins.jshint())//JS检查
        .pipe(plugins.jshint.reporter('default'))//检验后输出错误
        .pipe(plugins.uglify())//压缩JS
        .pipe(plugins.rename(JsPlugins.OutFile.Min))//重命名
        .pipe(gulp.dest(Url.App.Js));  // 输出
});

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//合并、压缩所有的JS,包括JS框架和自定义JS
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
gulp.task('js', ['min-jquery', 'js-min'], function () {//[为任务依赖，依赖的任务必须return]
    gulp.src([Url.App.Js + '/' + JsFrame.OutFile.Min, Url.App.Js + '/' + JsPlugins.OutFile.Min]) // 匹配文件
        .pipe(plugins.concat(JsPlugins.OutFile.All))//合并后的文件名
        .pipe(plugins.uglify())//压缩JS
        .pipe(plugins.rename(JsPlugins.OutFile.AllMin))//重命名
        .pipe(plugins.rev())
        .pipe(gulp.dest(Url.App.Js))                               //- 输出文件本地
        .pipe(plugins.rev.manifest())                               //- 生成一个rev-manifest.json
        .pipe(gulp.dest(Url.App.Js + '/rev'));                       //- 将 rev-manifest.json 保存到 rev 目录内
});
...
gulp.task('watch-script', function () {
    gulp.watch(Url.View.Js + '/' + JsPlugins.OutFile.Min, ['concat-scripts']);
});
...
gulp.task('concat-scripts', ['clean-js'], function () {
    return gulp.src([
        ...
        .pipe(gulp.dest(Url.View.Js))  // 输出
        .pipe(plugins.uglify())//压缩JS
        .pipe(plugins.rename(JsPlugins.OutFile.AllMin))
        .pipe(gulp.dest(Url.View.Js))
        .pipe(plugins.rev())
...
gulp.task('rev-js', function () {
    return gulp.src([Url.View.Js + '/rev/*.json', Url.View.Root + '/' + '*.html'])   //- 读取 rev-manifest.json 文件以及需要进行css名替换的文件
...
gulp.task('sass', function () {
    gulp.src(SassFile.InFile) // 匹配文件夹
